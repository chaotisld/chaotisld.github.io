...
// Función nueva: Detecta qué sección de la página se está viendo actualmente
function getVisibleSectionId() {
    const elements = document.querySelectorAll('section, header, footer, .map-preview-section');
    let visibleId = '';
    let minDistance = Infinity;

    elements.forEach(el => {
        const rect = el.getBoundingClientRect();
        // Calcula la distancia desde el centro del elemento al centro de la pantalla
        const distance = Math.abs(rect.top + rect.height / 2 - window.innerHeight / 2);
        
        if (distance < minDistance) {
            minDistance = distance;
            // Si la sección no tiene un ID en tu HTML, le asignamos uno dinámicamente
            if (!el.id) {
                el.id = 'seccion-' + Math.random().toString(36).substr(2, 9);
            }
            visibleId = el.id;
        }
    });
...
// Reemplaza tu antigua función toggleQRModal() por esta:
function toggleQRModal() {
    ...
    if (!isActive) {
        ...
        // Esperamos a que se abra la animación
        setTimeout(() => {
            ...
            // ¡La magia!: Construimos la URL con la sección exacta
            const visibleId = getVisibleSectionId();
            const baseUrl = window.location.href.split('#')[0]; // Quitamos anclas previas
            const targetUrl = visibleId ? `${baseUrl}#${visibleId}` : baseUrl;

            console.log("Generando QR para la ruta:", targetUrl);

            // Generamos el QR
            new QRCode(qrDiv, {
                text: targetUrl, // Mandará a la URL + la sección vista
                width: 180,
                height: 180,
                colorDark : "#000000", 
                colorLight : "#ffffff",
